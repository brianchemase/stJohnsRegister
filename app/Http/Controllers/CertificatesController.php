...
use App\Mail\CertificationEmail; // Import the Mailable class
...
class CertificatesController extends Controller
{
    ...
    public function MakeCertificate()
    {
      ...
      // $pdf->Cell(0, 10, $experyDate, 0, 1); // Add the text
       ...
       $pdfFilePath = $pdfDirectory . '/Training_certification.pdf';

       // Save the PDF to the file path
       $pdf->Output($pdfFilePath, 'F');

       // Send the PDF as an attachment in an email
       Mail::to($email)->send(new CertificationEmail($pdfFilePath, $client_names));
    ...
    public function getcert(Request $request)
    {
        ...
        exit;
                ...
                return redirect()->back()->with('error', "No record found for ID or Serial Number: $id_no");
            }
    }
    ...
    public function Emailcert($idnumber)
    {
        // return $request->all();
        $id_no = $idnumber;
